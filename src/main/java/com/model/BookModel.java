...
public class BookModel {
	...
	public Integer nextPK() throws DatabaseException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(ID) FROM bookings");
	...
	public BookBean findByName(String name) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM bookings WHERE firstName=?");
		BookBean bean = null;
		Connection conn = null;
	...
	public BookBean findByPK(long pk) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer("SELECT * FROM bookings WHERE ID=?");
		BookBean bean = null;
		Connection conn = null;
	...
	public void update(BookBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"UPDATE bookings SET FlightId=?,FlightName=?,FirstName=?,LastName=?,MobileNo=?,BookDate=?,EmailId=?,Address=?,NoOfPerson=?,Price=?,FinalPrice=?,"
	...
	public List list(int pageNo, int pageSize) throws ApplicationException {
		...
		ArrayList list = new ArrayList();
		StringBuffer sql = new StringBuffer("select * from bookings");
		// if page size is greater than zero then apply pagination
		if (pageSize > 0) {
			// Calculate start record index
			pageNo = (pageNo - 1) * pageSize;
